...
public class FormatterPropertyTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		for (int i = 0; i < propertyTypes.size(); i++) {
			propertyDicts.addAll(propertyDictService
	...
	private String createFormatterFunction() {
		...
		sb.append("result = ").append(arrayName).append("[rowData[\"")
		...
		sb.append(
				"\telse \tif(rowData['population." + name
						+ ".id']||(rowData.population && rowData.population.")
				.append(name).append("&&rowData.population.").append(name)
				.append(".id)){\n");
		// if(rowData["orgLevel.id"]){
		// return orgLevelData[rowData["orgLevel.id"]];
		// }
		sb.append("\tif(rowData[\"population.").append(arrayKey)
				.append("\"]){\n");
		sb.append("result = ").append(arrayName)
				.append("[rowData[\"population.").append(arrayKey)
				.append("\"]];\n");
		sb.append("\tif(result == null) return \"&nbsp;\"\n");
		sb.append("\t\treturn result;\n");
		sb.append("\t}\n");
