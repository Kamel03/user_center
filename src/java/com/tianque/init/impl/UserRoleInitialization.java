...
public class UserRoleInitialization implements Initialization
{
	...
	public UserRoleInitialization(PermissionService permissionService, OrganizationService organizationService, PropertyDictService propertyDictService)
	{
	...
	{
		List<Permission> permissions = permissionService.findAllPermissionsForPage(1, Integer.MAX_VALUE).getResult();
	...
	private Role addRole(String[] permissions)
	{
		...
		List<PropertyDict> roles = propertyDictService.findPropertyDictByDomainNameAndInternalId(PropertyTypes.WORKBENCH_TYPE, WorkBenchType.SUPER_LEVEL);
		if (roles != null && roles.size() > 0)
		{
			role.setWorkBenchType(roles.get(0));
		}
		List<PropertyDict> orgLevels = propertyDictService.findPropertyDictByDomainNameAndInternalId(OrganizationLevel.ORG_LEVEL_KEY, OrganizationLevel.CITY);
	...
	private User addUser()
	{
		...
		Map<String, String> map = Chinese2pinyin.changeChinese2Pinyin(user.getName());
		...
		User exitsUser = permissionService.findUserByUserName(user.getUserName());
		...
		if (exitsUser == null)
		{
			...
			permissionService.addUser(user);
